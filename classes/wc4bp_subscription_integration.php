...
class wc4bp_subscription_integration
{
	...
	public function __construct()
	{
		...
		add_action('init', array($this, 'add_rewrite_tags_and_rules'));
	}
	...
	public function wc4bp_my_account_process_shortcode_subscriptions_view_page($attr, $content = '')
	{
		...
		// Try to get it from query_vars
		if (!empty($GLOBALS['wp']->query_vars['view-subscription'])) {
			$id = $GLOBALS['wp']->query_vars['view-subscription'];
		}
		// Fallback: Parse from BuddyPress action variables
		elseif (!empty($bp->action_variables) && $bp->action_variables[0] === 'view-subscription' && !empty($bp->action_variables[1])) {
			$id = absint($bp->action_variables[1]);
		} else {
			return false;
		}
	...
	{
		add_rewrite_tag('%view-subscription%', '([^&]+)');
		add_rewrite_rule(
			'^members/([^/]+)/subscriptions/view-subscription/([^/]+)/?$',
			'index.php?author_name=$matches[1]&view-subscription=$matches[2]',
			'top'
		);
	}
}