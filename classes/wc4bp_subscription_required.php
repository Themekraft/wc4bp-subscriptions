...
class wc4bp_subscription_required
{
    ...
    public function remove_woo_footer()
    {
        ...
        if (isset($current_screen->id) && $current_screen->id === 'admin_page_wc4bp-subscription-install-plugins' && class_exists('WC_Admin')) {
    ...
    public static function is_current_active()
    {
        ...
        return is_plugin_active('wc4bp-subscription/wc4bp-subscription.php');
    }
    ...
    public function setup_and_check()
    {
        ...
        $config = array(
            'id' => 'wc4bp-subscriptions',                 // Unique ID for hashing notices for multiple instances of TGMPA.
            'menu' => 'wc4bp-subscriptions-install-plugins', // Menu slug.
            ...
            'strings' => array(
                'notice_can_install_required' => _n_noop(
                    ...
                    '<u>WooBuddy -> Subscriptions</u> plugin requires the following plugin: %1$s.',
                    '<u>WooBuddy -> Subscriptions</u> plugin requires the following plugins: %1$s.',
                    'wc4bp_subscription'
                ),
                'notice_can_install_recommended' => _n_noop(
                    /* translators: 1: plugin name(s). */
                    '<u>WooBuddy -> Subscriptions</u> plugin recommends the following plugin: %1$s.',
                    '<u>WooBuddy -> Subscriptions</u> plugin recommends the following plugins: %1$s.',
                    'wc4bp_subscription'
                ),
                'notice_can_activate_required' => _n_noop(
                    /* translators: 1: plugin name(s). */
                    'The following is a required plugin for <u>WooBuddy -> Subscriptions</u> and is currently inactive: %1$s.',
                    'The following is a required plugins for <u>WooBuddy -> Subscriptions</u> and they are currently inactive: %1$s.',
                    'wc4bp_subscription'
                ),
